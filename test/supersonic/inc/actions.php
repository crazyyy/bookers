...
function wpss_footer() {
	...
	if ( isset( $settings['cloudflare_api_key'] ) && isset( $settings['cloudflare_domain'] )  && isset( $_SERVER['HTTP_CF_RAY'] ) ) {
		...
		if ( strpos( $_SERVER['HTTP_HOST'],	$settings['cloudflare_domain'] ) === false ) {
			return;
		}
		//
...
function wpss_save_post( $post_id, $post = false, $update ) {
...
add_action( 'wpss_update_post', 'wpss_save_post', 10, 2 );
...
function wpss_home_url($url, $path, $orig_scheme, $blog_id) {
	...
	if ($add_supersonic_site_home == 1) {
		$url = add_query_arg(array('supersonic' => untrailingslashit(substr(admin_url()
					,strlen(trailingslashit(site_url()))))),$url);
	}
	$in_home_url = false;
	return $url;
}
...
function wpss_update( $post, $comment = 0 ) {
	...
	//
	if ( ( $comment == 0 && isset( $settings['refresh'][$post_type.'_this'] ) )
		|| ( $comment && isset( $settings['comments'] ) && isset( $settings['comments']['comment_this'] ) ) )
	{
	...
	if ( ( $comment == 0 && isset( $settings['refresh'][$post_type.'_home'] ) )
		|| ($comment && isset( $settings['comments'] ) && isset( $settings['comments']['comment_home'] ) ) )
	{
	...
	if ( ( $comment == 0 && isset( $settings['refresh'][$post_type.'_tax'] ) )
		|| ( $comment && isset( $settings['comments'] ) && isset( $settings['comments']['comment_tax'] ) ) )
	{
	...
	if ( ( $comment == 0 && isset( $settings['refresh'][$post_type.'_author'] ) )
		|| ( $comment && isset( $settings['comments'] ) && isset( $settings['comments']['comment_author'] ) ) )
	{
	...
	if ( ($comment == 0 && isset( $settings['refresh'][$post_type . '_date'] ) )
		|| ( $comment && isset( $settings['comments'] ) && isset( $settings['comments']['comment_date'] ) ) )
	{
	...
	if ( ( $comment == 0 && isset( $settings['refresh'][$post_type.'_other'] ) )
		|| ( $comment && isset( $settings['comments'] ) && isset( $settings['comments']['comment_other'] ) ) )
	{
	...
	if ( ( $comment == 0 && isset( $settings['refresh'][$post_type.'_add_clear'] ) )
		|| ( $comment && isset( $settings['comments'] ) && isset( $settings['comments']['comment_add_clear'] ) ) )
	{
