...
function wp_jit_default_scripts( &$scripts ) {
	...
	did_action( 'init' ) && $scripts->localize( 'quicktags', 'quicktagsL10n', new LabelsObject( array(
		'closeAllOpenTags' => array( 'func' => esc_attr(__('Close all open tags')) ),
		'closeTags' => array( 'func' => esc_attr(__('close tags')) ),
		...
		'toggleFullscreen' => array( 'func' => esc_attr( __('Toggle fullscreen mode') ) ),
		'textdirection' => array( 'func' => esc_attr( __('text direction') ) ),
		'toggleTextdirection' => array( 'func' => esc_attr( __('Toggle Editor Text Direction') ) )
	) ) );
	...
	if ( is_admin() ) {
		...
		did_action( 'init' ) && $scripts->localize( 'inline-edit-post', 'inlineEditL10n', new LabelsObject( array(
			...
			'comma' => trim( _x( ',', 'tag delimiter' ) ), // TODO: how to use such a "double" function with labelsobject?
		) ) );
