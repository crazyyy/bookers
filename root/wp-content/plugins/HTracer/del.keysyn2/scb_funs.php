...
//Теперь сами функции
	...
	function CreateProject($Domain,$MainKeys=false)
	{
		...
		{
			$ID=rand();
			$Search=mysql_query("select * from `scb_projects` where (`ID`='$ID') LIMIT 1") or die ('CreateProject_Select :'.mysql_error());
			if(mysql_num_rows($Search))
			{
				...
				mysql_query("insert into `scb_projects` SET `ID`='$ID', `Domain`='$Domain'") or die ('CreateProject_Insert :'.mysql_error());
		}
		if($MainKeys!==false)
