<?php //Функции для импорта из CSV
	function print_step3_csv_table()
	{
		...
		if(!$HaveCount)
		{
			echo '<h3 style="margin-bottom:0">Задание веса</h4>';
			echo 'В импортированной вами информации не было колонки Вес(число переходов), вам нужно выбрать правило определение веса.<br />
					<select name="count_source" id="count_source">
						<option value="wordstat_k">"WordStat" (без  уточнений)</option>
						<option value="wordstat">WordStat (во всех словоформах с уточнениями)</option>
						<option value="wordstat_kv">"!WordStat" (без уточнений и словоформ)</option>
						<option value="wordstat_v">!WordStat (без учета словоформ)</option>
						<option value="const_1">Все = 1</option>
						<option value="const_10">Все = 10</option>
						<option value="const_20">Все = 20</option>
						<option value="const_50">Все = 50</option>
						<option value="const_100">Все = 100</option>
						<option value="const_500">Все = 500</option>
						<option value="const_1000">Все = 1000</option>
					</select>
						<input type="button" value="Поехали!" id="ht_load_eva_btn" onclick="ht_load_eva()" />
						<b id="ht_load_eva_info"></b>
					<br />
				<br />	
				Ecли в ваших данных объеденены запросы с одинаковыми словоформами, то рекомендуется выбрать «"WordStat"».
				В противном случае лучше использовать вариант «"!WordStat"».<br />
				Если в ваших данных указаны только основные запросы (нет уточнений), то выбирайте варианты без двойных кавычек.
				Уточняющее запросы не будут отпарсены, но вес существующих будет скоректирован.
				<br /><br /><br />
			';
		}
		if(!$HaveURL)
		{
			echo '<h3 style="margin-bottom:0">Задание URL</h4>';
			echo '	В импортированных вами данных не было колонки URL.<br />
					Адреса страниц будут уточнены благодаря поисковой системе Google.<br />
					<!--<select name="q_url_source" id="q_url_source">
						<option value="y">Из Яндекса</option>
						<option value="g">Из Google</option>
						<option value="yg">Оба, приоритет у Яндекса</option>
						<option value="gy">Оба, приоритет у Google</option>
					</select>--><input type="button" id="ht_load_url_btn" onclick="ht_parse_se()" value="Поехали!"/>
					<b id="ht_load_url_info"></b>
					<br />
			<!--	<br />	
					Если вы выберите "Оба, приоритет у Яндекса",
						то скрипт спарсит поиск по сайту Яндекса и Google. 
					Если страницы, выдаваемой Google на первой позиции, нет в индексе Яндекса,
					а в Google есть страница выдаваемая Яндексом и при этом титлы страниц разные, 
					то скрипт выберет страницу выдаваемую Google. Тоже самое произойдет, если Яндекс не выдаст ни одного результата.
					Во всех остальных случаях будет выбрана страница выдаваемая Яндексом.<br />
					Если вы выберите "Оба, приоритет у Google", то алгоритм будет аналогичным.<br />
					-->
					
				<br /><br /><br />';
		}
		echo '
			Строки с нулевым весом с пустыми адресами или ключевыми словами импортированы не будут.<br />
			<table>
			<tr>
	...
	?>
		...
		<form method='post'>
			...
			Эта кнопка нужна только при возникновении ошибки, описанной выше. Чтобы перейти на следующий шаг, существует кнопка внизу страницы.<br />
			<b>Разделитель столбцов:</b>
			<select name='Comma' id='Comma' onchange='comma_changed()'>
				<option value='none'<?php if($Comma=='none') echo ' selected="selected"'?>>Нет</option>
				<option value=';'   <?php if($Comma==';')    echo ' selected="selected"'?>>Точка с запятой (;)</option>
				<option value=','   <?php if($Comma==',')    echo ' selected="selected"'?>>Запятая (,)</option>
				<option value='#'   <?php if($Comma=='#')    echo ' selected="selected"'?>>Решотка(#)</option>
				<option value='tab' <?php if($Comma=='tab'||$Comma=="\t")  echo ' selected="selected"'?>>Символ табуляции</option>
				<option value='lsp' <?php if($Comma=='lsp')  echo ' selected="selected"'?>>последний пробел</option>
				<option value='fsp' <?php if($Comma=='fsp')  echo ' selected="selected"'?>>первый пробел</option>
				<option value='HTracerSpecialComma'<?php if($Comma=='HTracerSpecialComma')  echo ' selected="selected"'?>>Системный</option>
				<option value='other'<?php if($is_other) echo ' selected="selected"'?>>Другой</option>
			...
			<input type='submit'  value='Изменить разделитель столбцов' /> 
			<br />
		</form>	<br />
	<?php	
	}
	...
	?>
		<select <?php echo $Size;?> name='col_role_<?php echo $i;?>' id='col_role_<?php echo $i;?>'>
			<option value='none'>Не имеет значения</option> 
			<option value='Query' <?php if($Role=='Query') echo 'selected="selected"';?>>Ключевое слово</option> 
			<option value='Count' <?php if($Role=='Count') echo 'selected="selected"';?>>Вес (число переходов)</option> 
			<option value='URL'   <?php if($Role=='URL')   echo 'selected="selected"';?>>URL страницы</option> 
		</select>
	<?php			
	}
